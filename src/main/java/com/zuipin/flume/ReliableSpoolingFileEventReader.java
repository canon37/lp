...
/**
 * @Title: ReliableSpoolingFileEventReader
 * @Package: com.zuipin.flume
 * @author: zengxinchao
 * @date: 2017年1月19日 下午5:32:32
 * @Description: 读取文件数据
 */
public class ReliableSpoolingFileEventReader implements ReliableEventReader {
	 ...
	 */
	private ReliableSpoolingFileEventReader(File spoolDirectory, String completedSuffix, String includePattern, String ignorePattern, String trackerDirPath,
			boolean annotateFileName, String fileNameHeader, boolean annotateBaseName, String baseNameHeader, String deserializerType, Context deserializerContext,
			String deletePolicy, String inputCharset, DecodeErrorPolicy decodeErrorPolicy, ConsumeOrder consumeOrder, boolean recursiveDirectorySearch) throws IOException {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Initializing {} with directory={}, metaDir={}, " + "deserializer={}",
					new Object[] { ReliableSpoolingFileEventReader.class.getSimpleName(), spoolDirectory, trackerDirPath, deserializerType });
		}
	...
	private List<File> getCandidateFiles(final Path directory) {
		...
		try {
			...
			logger.error("I/O exception occurred while listing directories. " + "Files already matched will be returned. " + directory, e);
		}
	...
	/**
	 * Return the filename which generated the data from the last successful {@link #readEvents(int)} call. Returns null if called before any file contents are read.
	 */
	...
	public List<Event> readEvents(int numEvents) throws IOException {
		...
		/*
		 * It's possible that the last read took us just up to a file boundary. If so, try to roll to the next file, if there is one. Loop until events is not empty or there is no
		 * next file in case of 0 byte files
		 */
		while (events.isEmpty()) {
			long current = new Date().getTime();
			if (current <= currentFile.get().getExpire()) {
				return Collections.emptyList();
			}
			logger.info("Last read took us just up to a file boundary && current file expired. " + "Rolling to the next file, if there is one.");
			retireCurrentFile();
			currentFile = getNextFile();
			if (!currentFile.isPresent()) {
				return Collections.emptyList();
	 ...
	 * <p>
	 * If these operations fail in a way that may cause duplicate log entries, an error is logged but no exceptions are thrown. If these operations fail in a way that indicates
	 * potential misuse of the spooling directory, a FlumeException will be thrown.
	 *
	...
	private void rollCurrentFile(File fileToRoll) throws IOException {
		...
		if (dest.exists() && PlatformDetect.isWindows()) {
			/*
			 * If we are here, it means the completed file already exists. In almost every case this means the user is violating an assumption of Flume (that log files are placed
			 * in the spooling directory with unique names). However, there is a corner case on Windows systems where the file was already rolled but the rename was not atomic. If
			 * that seems likely, we let it pass with only a warning.
			 */
			...
			if (renamed) {
				...
				/*
				 * If we are here then the file cannot be renamed for a reason other than that the destination file exists (actually, that remains possible w/ small probability due
				 * to TOC-TOU conditions).
				 */
				String message = "Unable to move " + fileToRoll + " to " + dest + ". This will likely cause duplicate events. Please verify that "
	...
	/**
	 * Returns the next file to be consumed from the chosen directory. If the directory is empty or the chosen file is not readable, this will return an absent option. If the
	 * {@link #consumeOrder} variable is {@link ConsumeOrder#OLDEST} then returns the oldest file. If the {@link #consumeOrder} variable is {@link ConsumeOrder#YOUNGEST} then
	 * returns the youngest file. If two or more files are equally old/young, then the file name with lower lexicographical value is returned. If the {@link #consumeOrder} variable
	 * is {@link ConsumeOrder#RANDOM} then cache the directory listing to amortize retreival cost, and return any arbitary file from the directory.
	 */
	...
	private Optional<FileInfo> openFile(File file) {
		try {
			...
			// sanity check
			Preconditions.checkState(tracker.getTarget().equals(nextPath), "Tracker target %s does not equal expected filename %s", tracker.getTarget(), nextPath);
			
			ResettableInputStream in = new ResettableFileInputStream(file, tracker, ResettableFileInputStream.DEFAULT_BUF_SIZE, inputCharset, decodeErrorPolicy);
			...
			return Optional.of(new FileInfo(file, deserializer, expire));
	...
	public static class Builder {
		...
		public ReliableSpoolingFileEventReader build() throws IOException {
			return new ReliableSpoolingFileEventReader(spoolDirectory, completedSuffix, includePattern, ignorePattern, trackerDirPath, annotateFileName, fileNameHeader,
					annotateBaseName, baseNameHeader, deserializerType, deserializerContext, deletePolicy, inputCharset, decodeErrorPolicy, consumeOrder, recursiveDirectorySearch);
		}
	}
